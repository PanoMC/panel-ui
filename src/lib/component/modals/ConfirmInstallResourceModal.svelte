      ...
      <div class="modal-body text-center">
        ...
        <div class="pb-3">
          ...
          {:else if $versionInfoObj?.action === 'UPDATE'}
            <i class="fa-solid fa-sync fa-3x d-block m-auto text-primary"></i>
          {:else if $versionInfoObj?.action === 'REINSTALL'}
             <i class="fa-solid fa-rotate-right fa-3x d-block m-auto text-primary"></i>
          {:else}
            <i class="fa-solid fa-download fa-3x d-block m-auto text-primary"></i>
          {/if}
        </div>
        ...
        <h5 class="mb-3">
          {#if $versionInfoObj?.action === 'INSTALL'}
             {$_('components.store-loading.install-title', { values: { name: $versionInfoObj.version.resourceTitle, version: $versionInfoObj.version.tag } })}
          {:else if $versionInfoObj?.action === 'UPDATE'}
             {$_('components.store-loading.update-title', { values: { name: $versionInfoObj.version.resourceTitle, version: $versionInfoObj.version.tag } })}
          {:else if $versionInfoObj?.action === 'DOWNGRADE'}
             {$_('components.store-loading.downgrade-title', { values: { name: $versionInfoObj.version.resourceTitle, version: $versionInfoObj.version.tag } })}
          {:else}
             {$_('components.store-loading.reinstall-title', { values: { name: $versionInfoObj.version.resourceTitle, version: $versionInfoObj.version.tag } })}
          {/if}
        </h5>
        <!-- Unverified Warning -->
        {#if $versionInfoObj && !$versionInfoObj.version.verified}
          <div class="alert alert-warning text-start d-flex gap-2 align-items-center mb-3" role="alert">
             <i class="fa-solid fa-triangle-exclamation"></i>
             <div>
               <strong>{$_('components.store-loading.unverified-resource-warning-title')}</strong>
               <br />
               <small>{$_('components.store-loading.unverified-resource-warning-description')}</small>
             </div>
          </div>
        {/if}
        ...
        {#if $versionInfoObj}
          <div class="card">
            <div class="card-body">
              <div class="row align-items-center">
                <div class="col-auto">
                   <div class="d-flex align-items-center justify-content-center bg-body-secondary rounded border overflow-hidden position-relative" style="width: 64px; height: 64px;">
                      {#if $type === 'ADDON'}
                          <img
                            src="{PANO_WEBSITE_API_URL}/resources/{$versionInfoObj.version.resourceId}/icon?preview=true"
                            alt="Icon"
                            width="64"
                            height="64"
                          ...
                          <i class="fa-solid fa-puzzle-piece text-primary fs-4" style="display: none;"></i>
                      {:else}
                         {#if $versionInfoObj.version.screenshots?.length > 0}
                            <img
                              src="{PANO_WEBSITE_API_URL}/resources/{$versionInfoObj.version.resourceId}/screenshots/{$versionInfoObj.version.screenshots[0]}?preview=true"
                              alt="Screenshot"
                              width="64"
                              height="64"
                ...
                <div class="col text-start">
                  ...
                  <div class="d-flex align-items-center gap-2 mb-1 flex-wrap">
                      <h5 class="mb-0 text-truncate" title={$versionInfoObj.version.resourceTitle}>
                          {$versionInfoObj.version.resourceTitle}
                      </h5>
                      {#if $versionInfoObj.version.verified}
                          <i class="fa-solid fa-circle-check text-success small" title="Verified"></i>
                      {/if}

                      {#if $versionInfoObj.action === 'UPDATE' || $versionInfoObj.action === 'DOWNGRADE'}
                          <span class="badge text-bg-secondary">
                              {$versionInfoObj.installed?.version || '?'} <i class="fas fa-arrow-right fa-xs mx-1"></i> {$versionInfoObj.version.tag}
                          </span>
                      {:else}
                  ...
                  <div class="small text-muted d-flex gap-3">
                      <span title="Author"><i class="fas fa-user me-1"></i> {$versionInfoObj.version.author}</span>
                      <span title="Size"><i class="fas fa-database me-1"></i> {formatBytes($versionInfoObj.version.size)}</span>
                  </div>
                </div>
          ...
          {:else}
             <p class="text-muted small mt-3 mb-0">
                {#if $versionInfoObj.action === 'UPDATE'}
                  {$_('components.store-loading.update-description', { values: { from: $versionInfoObj.installed.version, to: $versionInfoObj.version.tag } })}
                {:else if $versionInfoObj.action === 'REINSTALL'}
                  {$_('components.store-loading.reinstall-description', { values: { version: $versionInfoObj.version.tag } })}
                {:else}
...
<script>
  ...
  import ConfirmDowngradeModal, {
    show as showConfirmDowngradeModal,
  } from './ConfirmDowngradeModal.svelte';
