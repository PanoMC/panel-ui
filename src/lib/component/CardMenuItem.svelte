<li class="nav-item">
  <a
    ...
    class:active={typeof active !== 'undefined'
      ? active
      : matching($page.url.pathname, base + href, startsWith, matchingList)}><slot /></a>
</li>
...
<script>
  ...
  function matching(path, pathName, startsWith = false, matchingList) {
    return matchingList.length > 0
      ? matchingList.filter((item) => path + $page.url.search === base + item).length > 0
